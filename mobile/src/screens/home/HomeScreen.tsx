...
export default function HomeScreen({ navigation }: any) {
  ...
  function openService(item: typeof SERVICES[number]) {
    if (item.type === 'MAP') {
      ...
      navigation.navigate('CoursesIntro', { purpose: item.purpose, title: item.title });
    } else if (item.key === 'LOCATION') {
      navigation.navigate('RentalIntro');
    } else if (item.key === 'MWANA') {
      navigation.navigate('MwanaForm', { purpose: item.purpose, title: item.title });
  ...
  const menuItems: SideMenuItem[] = [
    ...
    { key: 'COURSES', icon: '🛍️', label: 'faites vos Courses avec Raha', onPress: () => goTo(() => navigation.navigate('CoursesIntro', { purpose: 'Course en ville', title: 'faites vos Courses avec Raha' })) },
    { key: 'SORTIES', icon: '🗓️', label: 'Planifiez vos sorties', onPress: () => goTo(() => navigation.navigate('PlanifierSortie')) },
    { key: 'LOCATION', icon: '🔑', label: 'Louer une voiture', onPress: () => goTo(() => navigation.navigate('RentalIntro')) },
