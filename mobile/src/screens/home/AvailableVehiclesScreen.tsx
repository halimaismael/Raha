...
export default function AvailableVehiclesScreen({ route, navigation }: any) {
  const {
    type, purpose, title, ownerType, scheduledDateISO,
    pickupName, destinationName, pickupLat, pickupLng,
  } = route.params || {};
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Si le lieu de prise en charge a déjà été renseigné (écran
        // précédent : saisie manuelle ou GPS), on le réutilise directement —
        // pas besoin de redemander la position ici.
      ...
      <View style={styles.header}>
        ...
        {!!pickupName && (
          <Text style={styles.routeSummary}>
            📍 {pickupName}{destinationName ? ` → ${destinationName}` : ''}
          </Text>
        )}
        {locationDenied && (
          ...
          renderItem={({ item }) => (
            <Pressable
              ...
              onPress={() => navigation.navigate('Booking', {
                ...
                initialPickupName: pickupName,
                initialDropoffName: destinationName,
              })}
            >
...
const styles = StyleSheet.create({
  ...
  routeSummary: { fontSize: 12.5, color: colors.charcoal, fontWeight: '700', marginTop: 8 },
